...
Registry::Registry(Identifier registry_id, std::vector<RegistryEntry> entries)
    : registry_id_(std::move(registry_id)), entries_(std::move(entries)) {}

const Identifier& Registry::get_registry_id() const { return registry_id_; }

const std::vector<RegistryEntry>& Registry::get_entries() const { return entries_; }

size_t Registry::get_size() const { return entries_.size(); }

ByteBuffer& Registry::write(ByteBuffer& buffer) const {
  buffer.write_string(registry_id_.to_string());
  buffer.write_varint(entries_.size());
